...
def b1_check(per_tok, per_seq, weights):
    ...
    print(f"  So the batch-24 row back-solves to ~{total/GiB:.2f} GiB of EFFECTIVE usable HBM.")
    print(f"  That is consistent with the gap between a card's advertised 24 GB and the")
    print(f"  memory actually exposed to a process, but this log does not independently")
    print(f"  establish it -- I have no L4 here to run nvidia-smi on.")
...
def b2():
    ...
    print("     100.00 (b48). This is the discriminating column. If the GPU were merely")
    print("     compute-saturated, per-token latency would rise with batch. It does not.")
    print("     Flat ITL alone would only suggest that the number of actively decoding")
    print("     sequences is not growing; combined with preempted_seqs (a directly")
    print("     MEASURED column: 0 -> 7 -> 23) and kv_cache_util pinned at 0.97, the")
    print("     reading is that ~25 sequences decode at the prior speed while the extra")
    print("     requests are evicted rather than served.")
